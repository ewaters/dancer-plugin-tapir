...

{
	...
	throws_ok { setup_thrift_handler } qr/Invalid thrift_idl file/, "Pass a non-existant thrift_idl file name";
}
...
throws_ok { setup_thrift_handler } qr/Failed to load MyWebApp::Handler/, "Handler doesn't load";

$INC{'MyWebApp/Handler.pm'} = undef;

throws_ok { setup_thrift_handler } qr/must be a subclass of Tapir/, "Handler invalid subclass";
...
throws_ok { setup_thrift_handler } qr/didn't call service/, "Handler didn't call service()";
...
throws_ok { setup_thrift_handler } qr/doesn't handle method createAccount/, "Missing methods";
...
lives_ok { setup_thrift_handler } "Setup now completes without errors";
